...
const enhancedSwaggerConfig = `import { Options } from 'swagger-jsdoc';
...
function updateSwaggerConfig(): boolean {
  try {
    const configPath = path.join(process.cwd(), 'src', 'config', 'swagger.config.ts');
    ...
    fs.writeFileSync(configPath, enhancedSwaggerConfig);
    console.log(`✅ Updated Swagger configuration with enhanced schemas and examples`);

    return true;
  } catch (error) {
    console.error('❌ Error updating Swagger config:', error);
    return false;
  }
}

async function main() {
  console.log('🔧 Updating Swagger configuration with enhanced schemas...\n');

  const success = updateSwaggerConfig();

  if (success) {
    console.log('\n✅ Swagger configuration updated successfully!');
    console.log('\n🔄 Next steps:');
    console.log('1. Restart the development server to load new configuration');
    console.log('2. Run npm run docs:generate to regenerate documentation');
    console.log('3. Check http://localhost:3000/api-docs for enhanced documentation');

    return 0;
  } else {
    console.log('\n❌ Failed to update Swagger configuration');
    return 1;
  }
}
